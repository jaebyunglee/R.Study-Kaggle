...
for(i in 1:30){
  ...
  lasso.rmse = sqrt(sum((exp.y -exp(predict(lasso.fit,as.matrix(part.new.valid.data[,-1]))))^2)/sum.y.2 )
  
  #ridge
  set.seed(2019)
  ridge.fit = cv.glmnet(as.matrix(part.new.train.data[,-1]),as.matrix(part.new.train.data[,1]),alpha=0)
  ridge.rmse = sqrt(sum((exp.y -exp(predict(ridge.fit,as.matrix(part.new.valid.data[,-1]))))^2)/sum.y.2 )
