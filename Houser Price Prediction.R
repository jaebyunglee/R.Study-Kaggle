...
# 패키지 : ggplt2, randomForest, caret, xgboost, e1071, corplot, gridExtra, glmnet, Matrix
# -----------------------------------------------------------------------------------------
...
f.train_y <- log(1+all_df$SalePrice[!is.na(all_df$SalePrice)])
...
rm(lasso, train_df, test_df, imp.var.names)

#제출 
lasso.submit <- data.frame(Id = rownames(lasso.pred), SalePrice = as.vector(exp(lasso.pred)))
write.csv(lasso.submit, "OUT/lasso.submit.csv", row.names = F)
rm(lasso.pred, lasso.submit)

#실제값과 예측값 비교
plot(exp(f.train_y), exp(lasso.pred2))
